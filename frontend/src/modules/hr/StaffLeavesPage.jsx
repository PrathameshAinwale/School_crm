...
export default function StaffLeavesPage() {
  ...
  const [previewPhoto, setPreviewPhoto] = useState(null); // { url, name, staffName, type, reason }
        ...
        {loading ? (
          ...
          leaveRequests.map((req) => (
            <div key={req.id || req.db_id} className="bg-white rounded-xl p-3.5 border border-gray-200/80 shadow-xs space-y-2.5">
              ...
              {req.photo_proof && (
                <div className="flex items-center gap-2 p-2 bg-rose-50/70 border border-rose-200 rounded-lg">
                  <img
                    src={req.photo_proof}
                    alt="Proof Preview"
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="text-[10px] font-bold text-rose-800 block truncate">Medical Certificate</span>
                    <span className="text-[9px] text-rose-600 truncate block">{req.photo_name || 'Proof.png'}</span>
                  </div>
                  <button
                    onClick={() => setPreviewPhoto({
                      url: req.photo_proof,
                      name: req.photo_name || 'Medical Proof',
                      staffName: req.name,
                      type: req.type,
                      reason: req.reason,
                    })}
                  ...
                  <tr key={req.id || req.db_id} className="hover:bg-gray-50/60 transition-colors">
                    ...
                    <td className="py-4 px-5">
                      <span className={`text-xs font-bold px-2.5 py-1 rounded-lg border ${
                        req.raw_type === 'SL' || req.type?.includes('Medical') || req.type?.includes('Sick')
                    ...
                    <td className="py-4 px-5">
                      <div className="text-xs text-gray-800 font-semibold">
                        ...
                        {req.endDate && req.endDate !== req.startDate && (
                          <span className="text-gray-500 block text-[11px]">to {req.endDate}</span>
                        )}
                      </div>
                      <span className="text-[10px] text-gray-400 font-medium">{req.days} day(s)</span>
                    </td>
                    ...
                    <td className="py-4 px-5 max-w-[280px]">
                      ...
                      {req.photo_proof && (
                        <button
                          ...
                          className="mt-1.5 inline-flex items-center gap-1.5 px-2.5 py-1 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-lg text-[11px] font-bold transition-colors cursor-pointer"
                        >
                          <LuImage className="w-3.5 h-3.5 text-rose-600" />
                          <span>View Medical Proof</span>
                        </button>
                      )}
                    </td>
      ...
      {previewPhoto && (
        <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto animate-fade-in">
          <div className="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl border border-slate-200 animate-scale-up">
            <div className="p-4 sm:p-5 bg-gradient-to-r from-rose-600 to-pink-600 text-white flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-xl bg-white/20 flex items-center justify-center">
                  <LuImage className="w-4 h-4 text-white" />
                </div>
                <div>
                  <h3 className="font-bold text-sm">Medical Certificate / Proof</h3>
                  <p className="text-[11px] text-rose-100">{previewPhoto.staffName} • {previewPhoto.type}</p>
                </div>
              </div>
              <button
            ...
            <div className="p-5 space-y-4">
              <div className="max-h-[380px] rounded-2xl overflow-hidden bg-slate-50 border border-slate-200 flex items-center justify-center">
                <img
              ...
              {previewPhoto.reason && (
                <div className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl text-xs text-slate-700">
                  <span className="font-bold block text-[10px] text-slate-500 uppercase tracking-wider mb-0.5">Stated Reason:</span>
                  {previewPhoto.reason}
                </div>
              )}
