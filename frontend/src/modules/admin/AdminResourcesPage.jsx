...
export default function AdminResourcesPage() {
  ...
  const handleAddSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const totalQty = formData.total_quantity === '' ? 0 : Number(formData.total_quantity) || 0;
      const payload = {
        ...formData,
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const totalQty = formData.total_quantity === '' ? 0 : Number(formData.total_quantity) || 0;
      const availQty = formData.available_quantity === '' ? 0 : Number(formData.available_quantity) || 0;
      const payload = {
        ...formData,
        total_quantity: totalQty,
                ...
                <div>
                  ...
                  <div className="flex items-center">
                    <button
                      type="button"
                      onClick={() => {
                        const cur = parseInt(formData.total_quantity, 10);
                        const next = isNaN(cur) ? 0 : Math.max(0, cur - 1);
                        setFormData({
                          ...formData,
                          total_quantity: next,
                          available_quantity: editingResource ? formData.available_quantity : next,
                        });
                      }}
                      className="w-9 h-9 flex items-center justify-center bg-slate-100 hover:bg-slate-200 active:bg-slate-300 text-slate-700 font-bold rounded-l-xl border border-r-0 border-slate-200 transition-colors cursor-pointer text-sm select-none"
                      ...
                      onBlur={() => {
                        if (formData.total_quantity === '') {
                          setFormData({
                            ...formData,
                            total_quantity: 0,
                            available_quantity: editingResource ? formData.available_quantity : 0,
                          });
                        }
                      }}
                      placeholder="0"
                      className="w-full text-center py-2 bg-slate-50 border border-slate-200 text-xs font-bold text-slate-800 focus:outline-none focus:border-cyan-500 font-mono"
                    />
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="w-9 h-9 flex items-center justify-center bg-slate-100 hover:bg-slate-200 active:bg-slate-300 text-slate-700 font-bold rounded-r-xl border border-l-0 border-slate-200 transition-colors cursor-pointer text-sm select-none"
