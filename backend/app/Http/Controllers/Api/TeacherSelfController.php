...
class TeacherSelfController extends Controller
{
    ...
    public function profile(Request $request)
    {
        ...
        return response()->json([
            ...
            'data' => [
                'id' => $teacher->id,
                'teacher_id' => $teacher->teacher_id ?: ('TCH-' . str_pad($teacher->id, 3, '0', STR_PAD_LEFT)),
                'first_name' => $teacher->first_name,
                'last_name' => $teacher->last_name,
                'full_name' => $teacher->full_name ?: trim("{$teacher->first_name} {$teacher->last_name}"),
                'email' => $teacher->email ?: ($user ? $user->email : null),
                'phone' => $teacher->phone ?: ($user ? $user->phone : null),
